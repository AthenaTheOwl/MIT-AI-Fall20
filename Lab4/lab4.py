...
family_rules = [same_identity, sibling_rule, child_rule, cousin_rule, grandparent_rule]
...
def backchain_to_goal_tree(rules, hypothesis):
    ...
    """
    consequent_matches = [hypothesis]  # always OR

    for r in rules:
        var = match(r.consequent(), hypothesis)

        if var is not None:
            var_antecedent = r.antecedent()
            antecedent_matches = []
            t = type(var_antecedent)
            next_hypothesis = populate(var_antecedent, var)

            if t == str:
                antecedent_matches.append(backchain_to_goal_tree(rules, next_hypothesis))
            else:
                for a in next_hypothesis:
                    antecedent_matches.append(backchain_to_goal_tree(rules, a))

            if t == OR:
                consequent_matches.append(OR(antecedent_matches))
            else:
                consequent_matches.append(AND(antecedent_matches))

    return simplify(OR(consequent_matches))
