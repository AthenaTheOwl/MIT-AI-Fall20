...
def check_alpha_equations(svm):
    ...
    for p in svm.training_points:
        if p in svm.support_vectors:
            check = vector_add(check, scalar_mult(p.classification * p.alpha, p.coords))
        track += p.classification * p.alpha
    if track == 0 and check == svm.w:
        return True
    return False
