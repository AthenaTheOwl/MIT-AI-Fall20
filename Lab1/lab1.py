...
def path_length(graph, path):
    ...
    if len(path) <= 1:
        ...
        for i in range(len(path) - 1):
            tmp_edge = graph.get_edge(path[i], path[i + 1])
            length = length + tmp_edge.length
        return length
...
def extensions(graph, path):
    ...
    check = graph.get_neighbors(path[-1])
    pathlist = []
    for node in check:
        newpath = path.copy()
        newpath.append(node)
...
def sort_by_heuristic(graph, goalNode, nodes):
    ...
    if len(nodes) == 1:
        return nodes
    nodes = break_ties(nodes)
    lengths = []
    for node in nodes:
        lengths.append(graph.get_heuristic_value(node, goalNode))
    combined = zip(lengths, nodes)
    return [a[1] for a in sorted(combined)]
...
def basic_dfs(graph, startNode, goalNode):
    ...
    while (solution):
        ...
        if (last == goalNode):
            ...
            new_paths = [ext for ext in extensions(graph, path) if not has_loops(ext)]
...
def basic_bfs(graph, startNode, goalNode):
    ...
    while (solution):
        ...
        if (last == goalNode):
            ...
            new_paths = [ext for ext in extensions(graph, path) if not has_loops(ext)]
            solution = solution + new_paths
    return None


def hill_climbing_sort(graph, goal, paths):
    last_node = []
    for path in paths:
        last_node.append(path[-1])
    last_node = sort_by_heuristic(graph, goal, last_node)
    sortedpaths = paths.copy()
    i = 0
...
def best_first_sort(graph, goal, paths):
    paths_lex = break_ties(paths)
    lengths = []
    for path in paths_lex:
        lengths.append(graph.get_heuristic_value(path[-1], goal))
    combined = zip(lengths, paths_lex)
    return [a[1] for a in sorted(combined)]


def branch_and_bound(graph, goal, paths):
    paths_lex = break_ties(paths)
    lengths = []
    for path in paths_lex:
        lengths.append(path_length(graph, path))
    combined = zip(lengths, paths_lex)
    return [a[1] for a in sorted(combined)]


def branch_and_bound_with_heuristic(graph, goal, paths):
    paths_lex = break_ties(paths)
    lengths = []
    for path in paths_lex:
        tmp = path_length(graph, path) + graph.get_heuristic_value(path[-1], goal)
        lengths.append(tmp)
    combined = zip(lengths, paths_lex)
    return [a[1] for a in sorted(combined)]
...
generic_best_first = [do_nothing_fn, True, best_first_sort, False]

generic_branch_and_bound = [do_nothing_fn, True, branch_and_bound, False]

generic_branch_and_bound_with_heuristic = [do_nothing_fn, True, branch_and_bound_with_heuristic, False]

generic_branch_and_bound_with_extended_set = [do_nothing_fn, True, branch_and_bound, True]

generic_a_star = [do_nothing_fn, True, branch_and_bound_with_heuristic, True]
...
def is_admissible(graph, goalNode):
    ...
    for node in graph.nodes:
        a = graph.get_heuristic_value(node, goalNode)
        b = path_length(graph,
...
def is_consistent(graph, goalNode):
    ...
    for edge in graph.edges:
        v = edge.startNode
        n = edge.endNode
        if graph.get_heuristic_value(v, goalNode) > (graph.get_heuristic_value(n, goalNode) + edge.length):
            return False
    return True
